...
import { useEffect, useRef, useState } from "react"; // Aggiunto useState
import { gsap } from "gsap";
import Link from "next/link"; // Aggiunto Link per navigare tra le pagine
  ...
  return (
    <div className="min-h-screen bg-white text-black p-10">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <header className="sticky top-0 z-50 py-4 flex justify-between items-start pointer-events-none">
          ...
          >
            <button className="text-base md:text-xl uppercase tracking-[0.2em] font-light mix-blend-difference text-white outline-none">
              Menu {isOpen ? "—" : "+"}
            </button>
            
            {/* La tendina con le tue nuove sezioni */}
            {isOpen && (
              <div className="mt-2 flex flex-col items-end space-y-4 pt-4">
                <Link href="/disegni" className="text-sm uppercase tracking-[0.2em] font-light mix-blend-difference text-white hover:opacity-50 transition-opacity">
                  disegni
                </Link>
                <Link href="/poetry" className="text-sm uppercase tracking-[0.2em] font-light mix-blend-difference text-white hover:opacity-50 transition-opacity">
                  poetry
                </Link>
              </div>
            )}
